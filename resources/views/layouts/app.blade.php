...
<body class="admin-shell min-h-screen bg-slate-950 text-slate-100 antialiased" style="--app-primary: {{ $appConfig?->color_primario ?? '#24a8ff' }}; --app-secondary: {{ $appConfig?->color_secundario ?? '#ec7519' }};">
    <div class="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(236,117,25,0.16),transparent_32%),radial-gradient(circle_at_top_right,rgba(36,168,255,0.16),transparent_30%),linear-gradient(180deg,#090d15_0%,#05070b_100%)]">
        <header class="sticky top-0 z-40 border-b border-white/10 bg-slate-950/90 shadow-2xl shadow-black/20 backdrop-blur">
            <nav class="flex w-full flex-wrap items-center justify-between gap-4 px-6 py-4 lg:px-10">
                <a href="{{ auth()->check() ? route('dashboard') : route('inicio') }}" class="flex items-center gap-3 text-lg font-black tracking-wide text-white">
                    @if ($appConfig?->logo_path)
                        <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($appConfig->logo_path) }}" alt="Logo" class="h-11 w-11 rounded-xl bg-white object-cover">
                    @else
                        <span class="grid h-11 w-11 place-items-center rounded-xl border border-white/30 bg-white text-xs font-black text-slate-950 shadow-lg shadow-sky-500/20">MB</span>
                    @endif
                ...
                <div class="flex flex-wrap items-center gap-3 text-sm font-bold">
                    @auth
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="m-0">
                            @csrf
                            <button type="submit" class="rounded-lg border border-sky-400 bg-sky-400 px-4 py-2 font-black text-slate-950 hover:border-sky-300 hover:bg-sky-300">
                                Salir
                            </button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="rounded-lg px-3 py-2 text-white/80 hover:bg-white/10 hover:text-white">Ingresar</a>
                        <a href="{{ route('register') }}" class="rounded-lg border border-sky-400 bg-sky-400 px-4 py-2 font-black text-slate-950 hover:border-sky-300 hover:bg-sky-300">Registrarse</a>
                    @endauth
                </div>
            </nav>
